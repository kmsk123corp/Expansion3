		...
		if (pPlayer:GetNumCities() > 0) then
			...
			for pResource in GameInfo.Resources() do
				...
				if (Game.GetResourceUsageType(iResourceLoop) == ResourceUsageTypes.RESOURCEUSAGE_STRATEGIC) then
                    ...
                    iNumStock = pPlayer:GetNumResourceStock(iResourceLoop);
					...
					if (bShowResource) then
						...
						-- strTempText = string.format("%i   ", text);
						...
						if (iNumAvailable > 0) then
							strTempText = text .. " " .. iNumStock .. " " .. "(+" .. iNumAvailable .. ")  ";
                                   -- strTempText = text .. " " .. iNumStock .. " " .. "[COLOR_POSITIVE_TEXT](+" .. iNumAvailable .. ")[ENDCOLOR]  ";
						elseif (iNumAvailable < 0) then
							strTempText = text .. " " .. iNumStock .. " " .. "[COLOR_WARNING_TEXT](" .. iNumAvailable .. ")[ENDCOLOR]  ";
                              else
                                   strTempText = text .. " " .. iNumStock .. "  ";
						end
		...
		if (Game.GetResourceUsageType(iResourceLoop) == ResourceUsageTypes.RESOURCEUSAGE_STRATEGIC) then
			...
			if (bShowResource) then
				...
				if (bThisIsFirstResourceShown) then
                    ...
                    if (iNumAvailable > 0) then
                         text_available = " ([COLOR_POSITIVE_TEXT]+" .. iNumAvailable .. "[ENDCOLOR]) ";
                    end
                    if (iNumAvailable < 0) then
                         text_available = " ([COLOR_WARNING_TEXT]" .. iNumAvailable .. "[ENDCOLOR]) ";
                    end

				strText = strText .. iNumStock .. " " .. pResource.IconString .. text_available .. Locale.ConvertTextKey(pResource.Description);
