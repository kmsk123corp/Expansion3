...
g_YourCultureSortOptions = {
	...
	{
		Button = Controls.VersaillesButton,
		Column = "VersaillesSort",
		DefaultDirection = "desc",
		CurrentDirection = nil,
		SortType = "numeric",
	},
...
function RefreshYourCulture()
    ...
    local buildings = {
		...
		BuildingEntry{
			BuildingClass = "BUILDINGCLASS_VERSAILLES",
			BuildingSortColumn = "VersaillesSort",
			IconControlNames = {"VersaillesGreatWork"},
			HighlightIconControlNames = {"VersaillesGreatWorkHL"},
			GhostIconControlNames = {"VersaillesGreatWorkGhost"},
			ColumnImageControl = Controls.VersaillesImage,
		},	
		BuildingEntry{
	...
	for building in GameInfo.Buildings() do
		...
		if((buildingClass.MaxGlobalInstances > 0 and building.GreatWorkCount > 0) or
			building.Type == "BUILDING_HERMITAGE" or building.Type == "BUILDING_OXFORD_UNIVERSITY" or building.Type == "BUILDING_SMITHSONIAN_INSTITUTION") then
